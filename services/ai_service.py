...
async def curate_products(scraped_items: list, user_query: str) -> list:
    ...
    system_prompt = (
        ...
        "For each product, extract key specs (formatted_specs) and write a concise, 2-sentence explanation ('why_it_fits_you') directly answering the user's request (e.g., 'Great value for money with high ratings. Ideal for daily coding needs.').\n"
        "Respond ONLY with a clean, valid JSON object matching this structure exactly:\n"
        "{\n"
        "  \"picks\": [\n"
        "    {\n"
        "      \"original_id\": <number>,\n"
